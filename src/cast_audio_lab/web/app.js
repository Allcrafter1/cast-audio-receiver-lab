...
async function api(path, method = "GET", body) {
  const response = await fetch(path.replace(/^\//, ""), {method, headers: {"Content-Type": "application/json"}, body: body === undefined ? undefined : JSON.stringify(body), credentials: "omit"});
...
$("scan").onclick = async () => {
  $("scan").disabled = true; message("Suche AirPlay-Ziele …");
  try {
    const result = await api("/api/scan", "POST", {}); $("candidates").replaceChildren();
    for (const candidate of result.candidates) {
      ...
      const button = action(candidate.already_imported ? "Bereits importiert" : "Deaktiviert importieren", async () => {
        ...
        await refresh(); $("candidates").replaceChildren(); message("Importiert. Zum Aktivieren die konfigurierte Ausgabe verwenden.");
